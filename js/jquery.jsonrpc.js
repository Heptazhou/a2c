	...
	$.extend({
		jsonRPC: {
			...
			request: function (method, options) {
				if (typeof options === "undefined") options = { id: 1 }
				if (typeof options.id === "undefined") options.id = 1
			...
			batchRequest: function (requests, options) {
				...
				$.each(requests, function (i, req) {
					...
					if (typeof req.id === "undefined") req.id = i + 1
				})
				this._validateRequestCallbacks(options.success, options.error)
			...
			_validateConfigParams: function (params) {
				if (typeof params === "undefined") throw "No params specified"
				else {
					if (params.endPoint && typeof params.endPoint !== "string") throw "endPoint must be a string"
					if (params.namespace && typeof params.namespace !== "string") throw "namespace must be a string"
				}
			},
			...
			_validateRequestParams: function (params) {
				if (!(params === null || typeof params === "undefined" || typeof params === "object" || $.isArray(params))) throw "Invalid params supplied for jsonRPC request. It must be empty, an object or an array."
				return true
			},

			_validateRequestCallbacks: function (success, error) {
				// Make sure callbacks are either empty or a function
				if (typeof success !== "undefined" && typeof success !== "function") throw "Invalid success callback supplied for jsonRPC request"
				if (typeof error !== "undefined" && typeof error !== "function") throw "Invalid error callback supplied for jsonRPC request"
				return true
			},
			...
			_doRequest: function (data, options) {
				...
				$.ajax({
					...
					beforeSend: function (xhr) {
						...
						if (authority) xhr.setRequestHeader("Authorization", "Basic " + $.base64.encode(authority))
					},
					error: function (json) {
			...
			_requestError: function (json, error) {
				if (typeof error !== "undefined" && typeof error === "function") error(this._response(json.responseText))
			},
