...
if (typeof ARIA2 == "undefined" || !ARIA2)
	var ARIA2 = (function () {
		...
		function get_error(result) {
			...
			else if (typeof result.error == "string") return result.error
			else if (result.error && result.error.message) return result.error.message
		}
		...
		function main_alert(_class, msg, timeout) {
			...
			if (timeout)
				window.setTimeout(function () {
					if ($("#main-alert").data("msg_id") == msg_id) $("#main-alert").fadeOut()
				}, timeout)
			return msg_id
		}
		...
		function get_title(result) {
			var dir = result.dir
			var title = "Unknown"
			if (result.bittorrent && result.bittorrent.info && result.bittorrent.info.name) title = result.bittorrent.info.name
			...
			} else if (result.files.length && result.files[0].uris.length && result.files[0].uris[0].uri) title = result.files[0].uris[0].uri

			if (result.files.length > 1) {
				var cnt = 0
				for (var i = 0; i < result.files.length; i++) if (result.files[i].selected == "true") cnt += 1
		...
		return {
			init: function (path, onready) {
				...
				if (jsonrpc_interface.indexOf("http") == 0) {
					...
					if (onready) onready()
					if ($("#main-alert").data("msg_id") == connect_msg_id) $("#main-alert").fadeOut()
					...
					jsonrpc_ws.onmessage = function (event) {
						...
						if ($.isArray(data) && data.length) {
							...
							if (ws_callback[data.id]) {
								if (data.error) ws_callback[data.id].error(data)
								else ws_callback[data.id].success(data)
				...
				} else main_alert("alert-error", "JSON-RPC Path错误, 只支持HTTP和WS协议")
			},
			...
			request_http: function (method, params, success, error) {
				if (error == undefined) error = default_error
				if (rpc_secret) {
					params = params || []
			...
			batch_request_http: function (method, params, success, error) {
				...
				$.each(params, function (i, n) {
					n = n || []
					if (!$.isArray(n)) n = [n]
					if (rpc_secret) n.unshift(rpc_secret)
			...
			_request_data: function (method, params, id) {
				...
				if (typeof params !== "undefined") dataObj.params = params
				return dataObj
			},
			...
			madd_task: function (uris, options) {
				...
				ARIA2.batch_request("addUri", params, function (result) {
					// console.debug(result)
					var error = new Array()
			...
			restart_task: function (gids) {
				$.each(gids, function (n, gid) {
					...
					$.each(result.files, function (n, e) {
						if (e.uris.length) uris.push(e.uris[0].uri)
					})
					if (uris.length > 0)
			...
			tell_active: function (keys) {
				...
				ARIA2.request("tellActive", keys, function (result) {
					// console.debug(result)
					if (select_lock) return
					...
					if (snapshot.sort().join(",") != active_tasks_snapshot) {
						...
						need_refresh = true
						if (auto_refresh && !select_lock) ARIA2.refresh()
					}

					result = ARIA2.status_fix(result.result)
					$("#active-tasks-table")
						.empty()
						.append(YAAW.tpl.active_task({ tasks: result }))
			...
			check_active_list: function () {
				ARIA2.request("tellActive", [["gid"]], function (result) {
					// console.debug(result)
					if (!result.result) main_alert("alert-error", "<strong>错误: </strong>RPC 通信错误.", 5000)
			...
			tell_waiting: function (keys) {
				...
				ARIA2.request("tellWaiting", params, function (result) {
					if (select_lock) return
					if (!result.result) main_alert("alert-error", "<strong>错误: </strong>RPC 通信错误.", 5000)
					...
					$("#waiting-tasks-table")
						.empty()
						.append(YAAW.tpl.other_task({ tasks: result }))
					$.each(result, function (n, e) {
						$("#task-gid-" + e.gid).data("raw", e)
					})
					bind_event($("#waiting-tasks-table"))

					if ($("#other-tasks .task").length == 0) $("#waiting-tasks-table").append($("#other-task-empty").text())
				})
			},
			...
			tell_stopped: function (keys) {
				...
				var params = [0, 1000]
				if (keys) params.push(keys)
				ARIA2.request("tellStopped", params, function (result) {
					// console.debug(result)
					if (select_lock) return
					if (!result.result) main_alert("alert-error", "<strong>错误: </strong>RPC 通信错误.", 5000)
					...
					if (finished_tasks_list === undefined) {
						...
						$.each(result, function (i, e) {
							if (e.status != "complete") return
							if (finished_tasks_list.indexOf(e.gid) != -1) return
							if (ARIA2.finish_notification) YAAW.notification("Aria2 控制台通知", e.title + " 已完成")
							finished_tasks_list.push(e.gid)
						})

					$("#stopped-tasks-table")
						.empty()
						.append(YAAW.tpl.other_task({ tasks: result.reverse() }))
					$.each(result, function (n, e) {
						$("#task-gid-" + e.gid).data("raw", e)
					})
					bind_event($("#stopped-tasks-table"))

					if ($("#waiting-tasks-table .empty-tasks").length > 0 && $("#stopped-tasks-table .task").length > 0) $("#waiting-tasks-table").empty()
				})
			},
			...
			status_fix: function (results) {
				for (var i = 0; i < results.length; i++) {
					...
					if (result.totalLength == 0) result.progress = "0.00"
					else result.progress = (((result.completedLength * 1.0) / result.totalLength) * 100).toFixed(2)
			...
			change_pos: function (gid, pos, how) {
				ARIA2.request("changePosition", [gid, pos, how], function (result) {
					// console.debug(result)
			...
			pause: function (gids) {
				...
				ARIA2.batch_request("pause", gids, function (result) {
					// console.debug(result)
					var error = new Array()
					$.each(result, function (i, n) {
						var error_msg = get_error(n)
					...
					if (error.length == 0) {
						...
						ARIA2.refresh()
					} else main_alert("alert-error", error.join("<br />"), 3000)
				})
			},

			unpause: function (gids) {
				if (!$.isArray(gids)) gids = [gids]
				ARIA2.batch_request("unpause", gids, function (result) {
					// console.debug(result)
					var error = new Array()
					$.each(result, function (i, n) {
						var error_msg = get_error(n)
			...
			remove: function (gids) {
				if (!$.isArray(gids)) gids = [gids]
				ARIA2.batch_request("remove", gids, function (result) {
					// console.debug(result)
					var error = new Array()
					$.each(result, function (i, n) {
						var error_msg = get_error(n)
			...
			remove_result: function (gids) {
				if (!$.isArray(gids)) gids = [gids]
				ARIA2.batch_request("removeDownloadResult", gids, function (result) {
					// console.debug(result)
					var error = new Array()
					$.each(result, function (i, n) {
						var error_msg = get_error(n)
			...
			get_options: function (gid) {
				ARIA2.request("getOption", [gid], function (result) {
					...
					if ($("#task-gid-" + gid).attr("data-status") == "active") $("#ib-options-form *[name]:not(.active-allowed)").attr("disabled", true)
				})
			},

			change_options: function (gid, options) {
				ARIA2.request("changeOption", [gid, options], function (result) {
					// console.debug(result)
			...
			get_peers: function (gid) {
				ARIA2.request("getPeers", [gid], function (result) {
					// console.debug(result)
					$("<tbody>")
						.appendTo($("<table>").appendTo($("#ib-peers").empty()))
			...
			pause_all: function () {
				ARIA2.request("pauseAll", [], function (result) {
					// console.debug(result)
					ARIA2.refresh()
			...
			unpause_all: function () {
				ARIA2.request("unpauseAll", [], function (result) {
					// console.debug(result)
					ARIA2.refresh()
			...
			purge_download_result: function () {
				ARIA2.request("purgeDownloadResult", [], function (result) {
					// console.debug(result)
					ARIA2.refresh()
			...
			get_global_option: function () {
				ARIA2.request("getGlobalOption", [], function (result) {
					if (!result.result) main_alert("alert-error", "<strong>错误: </strong>RPC 通信错误.", 5000)
			...
			change_global_option: function (options) {
				ARIA2.request("changeGlobalOption", [options], function (result) {
					if (!result.result) main_alert("alert-error", "<strong>错误: </strong>RPC 通信错误.", 5000)
					else main_alert("alert-success", "已保存", 2000)
				})
			},

			global_stat: function () {
				ARIA2.request("getGlobalStat", [], function (result) {
					if (!result.result) main_alert("alert-error", "<strong>错误: </strong>RPC 通信错误.", 5000)
					...
					var title = ""
					if (result.downloadSpeed > 0) title += "↓" + YAAW.tpl.view.format_size_0()(result.downloadSpeed) + "/s "
					if (result.uploadSpeed > 0) title += "↑" + YAAW.tpl.view.format_size_0()(result.uploadSpeed) + "/s "
			...
			get_status: function (gid) {
				ARIA2.request("tellStatus", [gid], function (result) {
					if (!result.result) main_alert("alert-error", "<strong>错误: </strong>RPC 通信错误.", 5000)
					...
					for (var i = 0; i < result.files.length; i++) {
						...
						if (file.uris && file.uris.length)
							for (var j = 0; j < file.uris.length; j++) {
								...
								if (result.uris.indexOf(uri) == -1) result.uris.push(uri)
							}
					}
					$("#ib-status").empty().append(YAAW.tpl.ib_status(result))
					$("#ib-files .file-list").empty().append(YAAW.tpl.files_tree(result.files))
					if ($("#task-gid-" + gid).attr("data-status") != "paused") $("#ib-file-save").hide()
					if (result.bittorrent) $("#ib-peers-a").show()
				})
			},
			...
			change_option: function (gid, options) {
				ARIA2.request("changeOption", [gid, options], function (result) {
					if (!result.result) main_alert("alert-error", "<strong>错误: </strong>RPC 通信错误.", 5000)
					else main_alert("alert-success", "选项更改成功!", 2000)
				})
			},
			...
			auto_refresh: function (interval) {
				...
				interval_id = window.setInterval(function () {
					ARIA2.global_stat()
					if (select_lock) {
						if (need_refresh) main_alert("", "任务列表已经改变。点击“刷新”按钮来更新任务列表。")
					} else {
						if (need_refresh) ARIA2.refresh()
						else ARIA2.tell_active()
					}
				}, interval)
				auto_refresh = true
			},
