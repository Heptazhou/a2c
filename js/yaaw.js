	...
	return {
		...
		aria2_init: function () {
			ARIA2.init(this.setting.jsonrpc_path, function () {
				if (YAAW.setting.add_task_option) $("#add-task-option-wrap").empty().append(YAAW.tpl.add_task_option(YAAW.setting.add_task_option))
				else ARIA2.init_add_task_option()
				ARIA2.refresh()
		...
		event_init: function () {
			...
			$(".task .task-name > span").live("click", function () {
				...
				if (task.hasClass("info-open")) YAAW.tasks.info_close()
				else {
			...
			$("#ib-files .ib-file-title, #ib-files .select-box").live("click", function () {
				if ($(this).parent().find(".select-box:first").hasClass("icon-ok")) $(this).parent().find(".select-box").removeClass("icon-ok")
				else $(this).parent().find(".select-box").addClass("icon-ok")
			})
			...
			$("#ib-file-save").live("click", function () {
				...
				if (indexes.length == 0) ARIA2.main_alert("alert-error", "至少有一个文件应该被选中或者只是停止此任务", 5000)
				else {
					var options = { "select-file": indexes.join(",") }
			...
			$("#select-all-btn").click(function () {
				if (selected_tasks) YAAW.tasks.unSelectAll()
				else YAAW.tasks.selectAll()
			})
			...
			if (window.applicationCache) {
				...
				$(document).ready(function () {
					if (appcache.status == appcache.IDLE) $("#offline-cached").text("cached")
				})
		...
		tpl: {
			...
			files_tree: function (files) {
				...
				function render(f) {
					...
					for (var k in f) {
						if (f[k]["_file"] !== undefined) continue
			...
			view: {
				bitfield: function () {
					...
					return function (text) {
						var len = text.length
						var result = ""
						for (var i = 0; i < len; i++) result += graphic[Math.floor(parseInt(text[i], 16) / 4)] + "&#8203;"
						return result
					}
				},
					...
					return function (text) {
						...
						for (var i = 0; i < 10; i++) {
							...
							for (var j = 0; j < part_len; j++) {
								if (i * part_len + j >= len) p += 16
								else p += parseInt(text[i * part_len + j], 16)
							}
				...
				format_size: function () {
					...
					return function (size) {
						...
						if (size == 0) return "0 KiB"
						else return size.toFixed(2) + " " + format_text[i]
					}
				},
				...
				format_size_0: function () {
					...
					return function (size) {
						...
						if (size == 0) return "0 KiB"
						else return size.toFixed(0) + " " + format_text[i]
					}
				},
				...
				format_time: function () {
					...
					return function (time) {
						...
						if (time > 0) result = time + "d" + result
						return result
					}
				},
				...
				format_date: function () {
					return function (u) {
						...
						return [[_pad(d.getFullYear()), _pad(d.getMonth() + 1), _pad(d.getDate())].join("-"), [_pad(d.getHours()), _pad(d.getMinutes()), _pad(d.getSeconds())].join(":")].join(" ")
					}
				},
				...
				format_peerid: function () {
					return function (peerid) {
						try {
							...
							if (peerid == "%00%00%00%00%00%00%00%00%00%00%00%00%00%00%00%00%00%00%00%00") return "unknown"
							var ret = unescape(peerid).split("-")
							for (var i = 0; i < ret.length; i++) if (ret[i].trim().length) return ret[i]
							return "unknown"
						}
					}
				},
			...
			submit: function (_this) {
				...
				$("#add-task-option input[name], #add-task-option textarea[name]").each(function (i, n) {
					...
					if (name && value) {
						...
						if ($(n).hasClass("input-save")) options_save[name] = String(value)
					}
				})
				...
				if (uri) {
					...
					if (file_type.indexOf("metalink") != -1) ARIA2.add_metalink(torrent_file, options)
					else ARIA2.add_torrent(torrent_file, options)
					YAAW.setting.save_add_task_option(options_save)
				}
			},
		...
		tasks: {
			check_select: function () {
				...
				if (selected.length + $(".info-box").length == 0) ARIA2.select_lock(false)
				else ARIA2.select_lock(true)
			...
			unSelectAll: function (notupdate) {
				...
				if (!notupdate) this.check_select()
			},
			...
			pause: function () {
				var gids = new Array()
				$(".tasks-table .task.selected").each(function (i, n) {
					if (n.getAttribute("data-status") == "active" || n.getAttribute("data-status") == "waiting") gids.push(n.getAttribute("data-gid"))
				})
			...
			unpause: function () {
				...
				$(".tasks-table .task.selected").each(function (i, n) {
					...
					if (status == "paused") gids.push(n.getAttribute("data-gid"))
					else if ("removed/error".indexOf(status) != -1) stopped_gids.push(n.getAttribute("data-gid"))
				})
			...
			remove: function () {
				...
				$(".tasks-table .task.selected").each(function (i, n) {
					if (remove_list.indexOf(n.getAttribute("data-status")) != -1) remove_gids.push(n.getAttribute("data-gid"))
					else gids.push(n.getAttribute("data-gid"))
				})
			...
			info: function (task) {
				...
				if (task.parents("#stopped-tasks-table").length) $("#ib-options-a").hide()
			...
			info_close: function (task) {
				...
				if ($(".tasks-table .task.selected").length == 0) ARIA2.select_lock(false)
				else ARIA2.select_lock(true)
			},
		},
		...
		contextmenu: {
			init: function () {
				$(".task")
					.live("contextmenu", function (ev) {
						...
						if ($(window).height() - ev.clientY < 200) contextmenu_position_y = ev.clientY - $("#task-contextmenu").height()
						if ($(window).width() - ev.clientX < 200) contextmenu_position_x = ev.clientX - $("#task-contextmenu").width()
						...
						if (status == "waiting" || status == "paused") $("#task-contextmenu .task-move").show()
						else $("#task-contextmenu .task-move").hide()
						...
						if (status != "waiting" || status != "active") $(".task-pause").hide()
						else $(".task-pause").show()
						return false
					})
					.live("mouseout", function (ev) {
						...
						if ($.contains(this, enteredElement) || $("#task-contextmenu").get(0) == enteredElement || $.contains($("#task-contextmenu").get(0), enteredElement)) return
						on_gid = null
			...
			remove: function () {
				var remove_list = ["active", "waiting", "paused"]
				if (on_gid) {
					if (remove_list.indexOf(status) !== -1) ARIA2.remove(on_gid)
					else ARIA2.remove_result(on_gid)
				}
				on_gid = null
			},
		...
		setting: {
			init: function () {
				...
				if (this.add_task_option) this.add_task_option = JSON.parse(this.add_task_option)
			...
			save: function () {
				...
				if (this.jsonrpc_history.indexOf(this.jsonrpc_path) == -1) {
					if (this.jsonrpc_history.push(this.jsonrpc_path) > 10) this.jsonrpc_history.shift()
				...
				if (this.finish_notification == 1) {
					if (!window.Notification) this.finish_notification == 0
