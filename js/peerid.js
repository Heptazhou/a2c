				...
				;(function (Buffer) {
					...
					module.exports = function (peerId) {
						...
						if (Buffer.isBuffer(peerId)) {
							...
							if (buffer.length !== 20) buffer = new Buffer(peerId, "hex")
						} else throw new Error("Invalid peerId must be Buffer or hex string: " + peerId)

						if (buffer.length !== 20) throw new Error("Invalid peerId length (hex buffer must be 20 bytes): " + peerId)
						...
						if ((client = utils.identifyAwkwardClient(peerId, buffer))) return client
					...
					function getSimpleClient(peerId) {
						for (var i = 0; i < customStyleClients.length; ++i) {
							...
							if (peerId.startsWith(client.id, client.position)) return client
						}
			...
			function (require, module, exports) {
				...
				module.exports = {
					...
					isShadowStyle: function (peerId) {
						...
						for (; lastVersionNumberIndex > 0; lastVersionNumberIndex--) if (peerId.charAt(lastVersionNumberIndex) !== "-") break
		...
		4: [
			function (require, module, exports) {
				...
				function Buffer(subject, encoding, noZero) {
					...
					if (encoding === "base64" && type === "string") subject = base64clean(subject)
					...
					else if (type === "number" && !Buffer._useTypedArrays && !noZero) for (i = 0; i < length; i++) buf[i] = 0
				...
				Buffer.prototype.copy = function (target, target_start, start, end) {
					...
					if (len < 100 || !Buffer._useTypedArrays) for (var i = 0; i < len; i++) target[i + target_start] = this[i + start]
